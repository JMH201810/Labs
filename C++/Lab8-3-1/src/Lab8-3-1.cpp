...
// Description : Operators: iostream
//============================================================================
...
ostream& operator<<(ostream &os, List &list) {	// overloaded insertion operator
	//printList(list);
	for (int i = 0; i < list.size(); i++)
	{
...
int main() {
	...
	input[0].value = 3;
	input[0].node = tree.AddChildLeft(nullptr, &(input[0].value));	// root

	input[1].value = 2;
	input[1].node = tree.AddChildLeft(input[0].node, &(input[1].value));	// left child

	input[2].value = 5;
	input[2].node = tree.AddChildRight(input[0].node, &(input[2].value));	// right child

	tree.Inorder(tree.get_RootPtr(), &list);
