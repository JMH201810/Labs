...
// Description : Interfaces and virtual functions: part 3
//============================================================================
	...
	bool isValid(std::string sentence){
		...
		for (unsigned int idxSenChar = 0; idxSenChar < sentence.length()-pattern.length()+1; idxSenChar++)	// loop over characters in the sentence
		{
			...
			for (unsigned int idxPatChar = 0; idxPatChar < pattern.length(); idxPatChar++)	// loop over characters in the pattern
			{
				...
				else if (pattern[idxPatChar] == 'A') // look for upper case character of the alphabet
				{
				...
				else if (pattern[idxPatChar] >= 'a'/* && pattern[idxPatChar] <= 'z'*/)	// look for lower case character of the alphabet
				{
					if ((sentence[idxSenChar+idxPatChar] >= 'a' && sentence[idxSenChar+idxPatChar] <= 'z')/*(sentence[idxSenChar+idxPatChar] == pattern[idxPatChar])
				...
				//					}
				//				}
				else if ((pattern[idxPatChar] == '$')/*(pattern[idxPatChar] >= '!' && pattern[idxPatChar] <= '/')
						...
						|| (pattern[idxPatChar] >= '{' && pattern[idxPatChar] <= '~')*/)	// look for any punctuation character
				{
					...
					if ((sentence[idxSenChar+idxPatChar] >= '!' && sentence[idxSenChar+idxPatChar] <= '/')
							|| (sentence[idxSenChar+idxPatChar] >= ':' && sentence[idxSenChar+idxPatChar] <= '>')
							|| (sentence[idxSenChar+idxPatChar] == '@') || (sentence[idxSenChar+idxPatChar] == '?')
							|| (sentence[idxSenChar+idxPatChar] >= '[' && sentence[idxSenChar+idxPatChar] <= '`')
							|| (sentence[idxSenChar+idxPatChar] >= '{' && sentence[idxSenChar+idxPatChar] <= '~'))	// look for any punctuation character
					{
...
private:
	MinLengthValidator* pminLenValidator;
	PatternValidator* pUCPattValidator, *pLCPattValidator, *pDPattValidator, *pSpecPattValidator;
public:
	PasswordValidator(){
		pminLenValidator = new MinLengthValidator(8);
		pUCPattValidator = new PatternValidator("A");
		pLCPattValidator = new PatternValidator("a");
		pDPattValidator = new PatternValidator("D");
		pSpecPattValidator = new PatternValidator("$");
	}

	~PasswordValidator(){
		delete pSpecPattValidator;
		delete pDPattValidator;
		delete pLCPattValidator;
		delete pUCPattValidator;
		delete pminLenValidator;
	}
	...
	bool isValid(std::string input){
		...
		if (pminLenValidator->isValid(input)
				&& pUCPattValidator->isValid(input)
				&& pLCPattValidator->isValid(input)
				&& pDPattValidator->isValid(input)
				&& pSpecPattValidator->isValid(input))
		{
			return true;
		}
